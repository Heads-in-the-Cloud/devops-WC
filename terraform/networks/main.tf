...
resource "aws_secretsmanager_secret_version" "secret_string" {
  ...
  secret_string = jsonencode(merge({"${secrets_key_subnet_group_id}"         = aws_db_subnet_group.private-subnet-group.id},
                                   {"${secrets_key_vpc_id}"                  = aws_vpc.my_vpc.id},
                                   {"${secrets_key_public_subnet_id}"        = aws_subnet.public_1.id}))
}
