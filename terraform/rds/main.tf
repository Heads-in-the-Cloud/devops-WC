...
resource "aws_secretsmanager_secret_version" "secret_string" {
  ...
  secret_string = jsonencode(merge({"${var.secrets_key_password}" = random_password.db_password.result},
                                   {"${var.secrets_key_host}"     = aws_db_instance.rds.address},
                                   {"${var.secrets_key_jwt_key}"  = random_password.secret_key.result},
                                   {"${var.secrets_key_user}"     = var.db_user},
